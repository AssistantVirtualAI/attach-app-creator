...
-- Update the setup_new_user_organization function to use 14-day trial
CREATE OR REPLACE FUNCTION public.setup_new_user_organization(_user_id uuid, _user_email text, _full_name text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Create billing config with 14-day trial
  _trial_ends_at := now() + interval '14 days';
  
  INSERT INTO public.billing_config (organization_id, plan_tier, trial_ends_at, subscription_status)
  VALUES (_org_id, 'free', _trial_ends_at, 'trialing');
