...
-- Defense-in-depth: revoke sensitive columns from authenticated/anon roles.
-- Server (service_role) and edge functions retain access.
...
REVOKE SELECT (
  ...
  fusionpbx_server_url, fusionpbx_domain_name, fusionpbx_domain_uuid,
  api_key, backend_domain
...
REVOKE SELECT (password_hash, password_reset_token, password_reset_expires_at)
  ON public.client_credentials FROM authenticated, anon;
REVOKE SELECT (password_hash, password_reset_token, password_reset_expires_at)
  ON public.client_member_credentials FROM authenticated, anon;

REVOKE SELECT (email, code_hash) ON public.two_factor_otps FROM authenticated, anon;
