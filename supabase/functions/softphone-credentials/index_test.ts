...
const fnUrl = baseUrl ? `${baseUrl.replace(/\/$/, "")}/functions/v1/softphone-credentials` : "";

Deno.test("softphone-credentials: rejects unauthenticated requests", async () => {
  if (!fnUrl) return;
  const res = await fetch(fnUrl, { method: "GET" });
  if (res.status !== 401 && res.status !== 405) throw new Error(`expected 401/405, got ${res.status}`);
});

Deno.test("softphone-credentials: answers CORS preflight", async () => {
  if (!fnUrl) return;
