...
const fnUrl = baseUrl ? `${baseUrl.replace(/\/$/, "")}/functions/v1/mobile-me` : "";

Deno.test("mobile-me: rejects unauthenticated requests", async () => {
  if (!fnUrl) return;
  const res = await fetch(fnUrl, { method: "GET" });
  if (res.status !== 401 && res.status !== 405) throw new Error(`expected 401/405, got ${res.status}`);
});

Deno.test("mobile-me: answers CORS preflight", async () => {
  if (!fnUrl) return;
