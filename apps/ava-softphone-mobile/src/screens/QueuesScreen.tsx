...
export default function QueuesScreen() {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '14px 14px 20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '0 2px 10px' }}><div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', borderRadius: 12, background: 'rgba(255,255,255,0.7)', border: `1px solid ${colors.border}` }}><Search size={14} color={colors.mutedSilver} /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search queues…" style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontSize: 13, color: colors.textIce }} /></div><button onClick={refresh} disabled={loading} style={{ padding: '8px 12px', borderRadius: 999, border: `1px solid ${colors.border}`, background: 'rgba(255,255,255,0.7)', color: colors.lemtelBlue, fontSize: 11, fontWeight: 800, cursor: 'pointer' }}>{loading ? '…' : '↻'}</button></div>
      <div style={{ fontSize: font.xs, color: colors.mutedSilver, margin: '0 2px 10px' }}>{joinedCount} active · {Object.keys(states).length} joined · live sync{lastSyncedAt ? ` · ${new Date(lastSyncedAt).toLocaleTimeString()}` : ''}</div>
