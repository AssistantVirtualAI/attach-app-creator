...
const MODULES: { id: Module; label: string; desc: string; tone: 'violet' | 'cyan' | 'gold' | 'success' | 'danger' }[] = [
  ...
  { id: 'queues',       label: 'Queue Builder',     desc: 'Routing strategy, overflow, hold messaging.',       tone: 'success' },
  { id: 'agents',       label: 'Voice Agents',      desc: 'Assign AVA voice agents to numbers and IVRs.',      tone: 'danger' },
];
...
export default function AIScreen() {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '14px 14px 20px' }}>
      ...
      <h1 style={{ fontSize: font.xxl, color: colors.textIce, margin: '0 0 6px', fontWeight: 800, letterSpacing: -0.3 }}>AI Workspace</h1>
      <p style={{ fontSize: font.sm, color: colors.mutedSilver, margin: 0, lineHeight: 1.5 }}>
        Intelligence, automation, and voice-agent control for your business communications.
      </p>
...
function Intelligence() {
  const rows = [
    ...
    { who: 'Vincent K.',     score: 41, sent: 'Negative', topic: 'Complaint' },
  ];
  return (
    <AIPanel title="Latest calls analyzed" accent={colors.avaViolet}>
      {rows.map((r, i) => (
...
function Actions() {
  const items = [
    { who: 'Marie Tremblay', task: 'Send pricing PDF', due: 'Today' },
    { who: 'Acme Corp',      task: 'Reschedule demo to Thursday', due: 'Tomorrow' },
    { who: 'Sophie B.',      task: 'Follow up on onboarding', due: '+3 days' },
  ];
  return (
    <AIPanel title="Action items" accent={colors.avaCyan}>
      {items.map((it, i) => (
...
function Greetings() {
  ...
  return (
    <Card padded={true} accent="gold">
      <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1.5, color: colors.signalGold, textTransform: 'uppercase', marginBottom: 8 }}>Greeting Studio</div>
      ...
      <PrimaryButton onClick={gen} disabled={busy} style={{ marginTop: 10, width: '100%' }}>
        {busy ? 'Generating…' : '✨ Generate with AVA'}
      </PrimaryButton>
      {out && (
        <div style={{ marginTop: 14, padding: 14, borderRadius: 12, background: colors.midnight2, border: `1px solid ${colors.borderAI}` }}>
          <div style={{ fontSize: 10, color: colors.avaCyan, fontWeight: 800, letterSpacing: 1.4, textTransform: 'uppercase', marginBottom: 6 }}>Generated Script</div>
          <p style={{ fontSize: font.base, color: colors.textIce, lineHeight: 1.55, margin: 0 }}>{out}</p>
          <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
            <GhostButton tone="gold">🎙 Synthesize</GhostButton>
            <GhostButton tone="cyan">Save as IVR</GhostButton>
          </div>
        </div>
      )}
    </Card>
...
function Queues() {
  ...
  return (
    <AIPanel title="Queue recommendations" accent={colors.success}>
      {rows.map((r, i) => (
...
function Agents() {
  const agents = [
    { name: 'AVA Reception',         voice: 'ElevenLabs · Rachel', assigned: ['+1 514 555 0100', 'IVR → 0'] },
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '4px 4px 10px' }}>
        <div style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: 1.5, color: colors.danger, textTransform: 'uppercase' }}>Active voice agents</div>
        <GhostButton tone="violet" style={{ padding: '6px 10px' }}>+ New</GhostButton>
      </div>
      {agents.map((a, i) => (
