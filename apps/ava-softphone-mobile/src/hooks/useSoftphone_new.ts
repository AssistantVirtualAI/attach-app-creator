...
// SIPConfig inline to avoid importing jssipProvider (which pulls in JsSIP)
...
// Inline types to avoid pulling in useSoftphone.ts → jssipProvider → WebRTC
export type SIPStatus = 'idle' | 'connecting' | 'registered' | 'retrying' | 'error';
export type CallState = 'idle' | 'ringing' | 'ringing-in' | 'active' | 'held' | 'ended';
export type UseSoftphoneReturn = any;
